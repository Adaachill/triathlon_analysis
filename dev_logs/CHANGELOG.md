...
## 2026-05-15: 個人ページのレイアウト整理（伸び代カード優先・What-if折りたたみ化）
**コミット:** `(本PR)`
**ブランチ:** claude/athlete-layout-Kp9w

### 変更内容
- `frontend/src/pages/AthleteDetail.tsx`:
  - セクション順を変更。`GrowthCards`（伸び代カード）を分析セクションの先頭へ。`SegmentRadar`（レーダー）を2番目に
  - `WhatIfSimulator` を分析セクション先頭から削除し、ページ最下部（レース履歴テーブルの後）に移動。`<details>` で折りたたみ化（デフォルト閉）
- `frontend/src/pages/pages.css`: `.whatif-details` / `.whatif-summary-toggle` スタイル追加。折りたたみ内では WhatIfSimulator の外側 margin/border を打ち消し

### 変更意図・背景
#56/#57/#58 をマージした結果、個人ページの情報量が過多になり、特に What-if シミュレータが最上部にあることで「メリットが伝わりにくい・本来見たい伸び代分析が埋もれる」状態だった。

価値の高い順（伸び代カード → レーダー → 推移グラフ → 履歴）に並べ替え、補助的な What-if は最下部の opt-in 折りたたみに格下げして初期表示の情報密度を下げた。

### 技術的決定事項
- **削除ではなく折りたたみ**: What-if を使いたいユーザーの導線は残しつつ、デフォルトでは畳んで視認ノイズを除去
- **最下部配置**: レース履歴テーブルの後（最も探索的なツールなので、データを見た後に「では改善したら？」と試す自然な動線）
- **`<details>` ネイティブ要素採用**: JS state 不要・アクセシブル。既存の用語解説アコーディオン（Guide.tsx）と同じパターンで一貫性

### 残課題・次のステップ
- なし（レイアウト調整のみ、ロジック変更なし）
