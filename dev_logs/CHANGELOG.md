...
## 2026-05-15: ナビゲーション整理（手動アップロードをフォールバック化）
**コミット:** `(本PR)`
**ブランチ:** claude/nav-cleanup-Mn4r

### 変更内容
- `frontend/src/App.tsx`: メインナビから `/admin` (`📤 レース結果アップロード`) を削除。`/admin/wt-import` を `📥 データ取得` にリネームして主要メニューに昇格。`/admin` ルートそのものは残してURL直アクセス・フォールバックリンクから到達可能
- `frontend/src/pages/WtImport.tsx`: ページタイトルを `📥 データ取得（World Triathlon 過去大会）` に変更。ページ最下部に「非公式レースは手動アップロード」へのフォールバックリンクを追加
- `frontend/src/pages/WtImport.css`: `.wti-fallback` スタイル追加
- `frontend/src/pages/Admin.tsx`: 手動アップロードページに「WT公式大会は📥データ取得から自動取込推奨」の青色お知らせを追加
- `frontend/src/pages/pages.css`: `.admin-redirect-note` スタイル追加

### 変更意図・背景
World Triathlon の Algolia API 連携で公式大会は自動インポートできるようになり、手動アップロードはほぼ不要になっていた。しかし「📤 レース結果アップロード」がナビの一等地に居座っていたため、初見ユーザーが「データ取り込みは Excel 必須」と誤解する導線になっていた。

メニュー整理により：
1. 新規ユーザーはまず 📥 データ取得 から World Triathlon 大会を一括取り込み
2. 非公式レース等の例外的なケースは WtImport ページ末尾のフォールバックリンクから /admin へ
3. /admin ページ自体にも「WT公式大会は自動取込推奨」の誘導を追加し、誤って到達した場合も適切なページへ案内

### 技術的決定事項
- **`/admin` ルートは削除しない**: 既存リンク・ブックマーク・直接URL入力を壊さないため。ナビから外すだけ
- **WtImport ページ末尾にフォールバックリンク**: 「データ取得」を試してから「やっぱり手動が必要」と気づくユーザー導線を確保
- **Admin ページの青色ノート**: 既存の黄色「⚠️ ファイル取得方法」とは別の色で「誘導」と「警告」を視覚的に分離
...
### 変更内容
- `frontend/src/components/WhatIfSimulator.tsx`: 個人ページに統合する What-if シミュレータコンポーネントを新規追加
  - 5本のスライダー（Swim / T1 / Bike / T2 / Run）でセグメントごとに -120s〜+60s の Δ 補正を入力可能
  - 合計タイム（標準化）と予想順位がリアルタイムで再計算
  - セグメント別の元順位 → シミュレート順位もインライン表示
  - リセットボタン付き
- `frontend/src/components/WhatIfSimulator.css`: スライダー・サマリ・カード専用スタイル
- `frontend/src/pages/AthleteDetail.tsx`: 個人ページに What-if セクションを追加。カテゴリランキングを並列フェッチ

### 変更意図・背景
従来は「自分の強みは Bike」というのは分かっても、「Bike を 30秒縮めたら何位まで上がるか？」という戦略的な問いに答えられなかった。

What-if シミュレータは「どこを縮めれば順位がどう動くか」を体感的に理解させ、トレーニング配分の意思決定支援を提供する。

### 技術的決定事項
- **計算方法**: athlete の `strength` に Δ_total = Σ(Δ_seg) を加算した値を新 strength とし、カテゴリランキングの他選手 strength と比較して新順位を算出
  - ALS の strength はセグメント合計と厳密には一致しないが、シミュレーション目的では十分な近似
- **セグメント別新順位**: 各セグメントについて `strength_seg + Δ_seg` をカテゴリ内の同セグメント strength と比較して算出。「Bike が 8位 → 3位に上がる」のような具体的な変化を提示
- **スライダー範囲**: Swim/Run は -120〜+60s、Bike は -180〜+90s、T1/T2 は -15〜+15s で現実的なレンジに制限
- **リアルタイム再計算**: `useMemo` で依存変化のみ再計算。スライダー操作のレスポンス維持
- **データ不足時の対応**: 全セグメント strength が揃っていない場合は「利用不可」表示。カテゴリランキングが未取得時は「順位再計算は取得後」と表示

### 残課題・次のステップ
- ALS strength と セグメント合計のズレを補正する「再正規化モード」は未実装。需要があれば次PRで
- 実走タイム（コース難易度込み）を表示するモードは未対応。現在は標準化タイム前提
## 2026-05-15: 選手分析「伸び代カード＋レーダーチャート」追加
**コミット:** `(本PR)`
**ブランチ:** claude/growth-radar-y8Kt

### 変更内容
- `frontend/src/components/GrowthRadar.tsx`: 2つのコンポーネントを新規追加
  - `GrowthCards`: セグメント（Swim/T1/Bike/T2/Run）ごとに「カテゴリ内順位」「TOP3とのギャップ」「TOP10とのギャップ」をカード表示。最大の強み・伸び代を自動判定してバッジ表示
  - `SegmentRadar`: 5軸レーダーチャート（recharts `RadarChart`）でカテゴリ内パーセンタイルを可視化。TOP10ライン（=上位約20%）を重ねて表示
- `frontend/src/components/GrowthRadar.css`: カード・メーター・レーダー専用スタイル
- `frontend/src/pages/AthleteDetail.tsx`: 個人ページに上記2セクションを追加。カテゴリランキングを並列フェッチ（`api.getRankings(program, 200)`）して同カテゴリ比較に利用

### 変更意図・背景
従来のAthleteDetailは「過去レースのタイム推移」「期間別平均」など過去を振り返る情報が中心で、「次どこを鍛えるべきか」「自分の強みは何か」を読み取りにくかった。

選手・コーチが**伸び代の優先順位**を一目で判断できるようにし、トレーニング計画や戦略立案の根拠データとして活用できるようにした。

### 技術的決定事項
- **パーセンタイル方式**: 各セグメントで「カテゴリ内の何%の選手より速いか」を 0〜100 のスコアで表示。レース距離・カテゴリで桁が違うタイム値より直感的に比較可能
- **TOP3 / TOP10 平均との差**: 絶対的なギャップを秒数で提示することで「TOP3まであと42秒」のように具体的な目標化が可能
- **最大の伸び代の自動判定**: `gapToTop3` が最大のセグメントを「伸び代」、`percentile` が最大のセグメントを「強み」としてバッジ強調。コーチ視点で即座に着目すべき箇所を提示
- **並列フェッチ**: 個人データと同時にランキングを取得することでブロッキングを回避（ランキング取得失敗時もメイン情報は表示）
- **backend 変更なし**: 既存の `/rankings/top?limit=200` で十分。カテゴリ内 athletes は 200 を超えないため `le=500` の上限内で安全
- **メーター視覚化**: 帯にTOP3（金色）とTOP10（暗色）のマーカーを置き、自分の位置との関係を一目で把握可能に

### 残課題・次のステップ
- レーダーの軸ラベルが長いとモバイルで重なる可能性あり。将来 SegLabel を icon のみにする等の検討余地
- TOP3／TOP10 が3人/10人未満のカテゴリでは比較精度が落ちる。将来「データ件数が少ない時の警告表示」を検討
- 次PRで「What-if シミュレータ」を実装予定
...
### 変更内容
- `CLAUDE.md`: 「モバイル互換性ルール」セクションを「マージ前チェックルール」の直後に追加

### 変更意図・背景
PR #51（バンプチャート iOS 表示不具合）の教訓を開発ルールとして文書化。iOS / Android で同種の問題を繰り返さないために、DOM サイズ取得・インタラクション実装・SVG 落とし穴・PR チェックリストを明文化した。

### 技術的決定事項
- iOS はすべてのブラウザが WebKit（WKWebView）を使うため「Safari 1台で iOS 全ブラウザをカバー」と明記
- Android は Chrome(Blink)・Firefox(Gecko) と複数エンジンが混在する点を表で整理
- `ResizeObserver` を DOM サイズ取得の標準パターンとして採用し、コード例付きで禁止パターンと推奨パターンを示した
